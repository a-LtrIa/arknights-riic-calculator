...
class 发电站效率计算器:
    ...
    def 计算效率(self) -> dict:
        ...
        for 干员 in self.干员实例列表:
            效率 = 干员.计算效率()
            # 发电站干员提供充能速度加成（属性名为"充能速度"）
            if hasattr(干员, '基本属性') and '充能速度' in 干员.基本属性:
                干员充能加成 += 干员.基本属性['充能速度']
            详情.append({
        ...
        return {
            ...
            "进驻人员": len(self.进驻干员),
            "日估算收益": {
                "日发电量": f"{POWER_STATION_POWER[self.等级]}",
                "日充能": f"+{总充能速度:.1f}%",
            },
            "干员详情": 详情,
        }
...
class 会客室效率计算器:
    ...
    def 计算效率(self) -> dict:
        ...
        return {
            ...
            "日估算收益": {
                "线索搜集速度": f"+{干员线索加成:.1f}%",
                "好友位": RECEPTION_FRIEND_SLOTS[self.等级],
            },
            "干员详情": 详情,
        }
...
class 办公室效率计算器:
    ...
    def 计算效率(self) -> dict:
        ...
        全局办公室加成 = 0.0
        if self.特殊变量:
            全局办公室加成 = self.特殊变量.获取变量("全局办公室加成")

        # 总联络速度 = 基础 + 干员加成 + 全局办公室加成
        总联络速度 = 小计联络速度 + 全局办公室加成
        ...
        return {
            ...
            "基础联络速度": f"+{基础联络}%",
            "干员联络加成": f"+{干员联络加成:.1f}%",
            "全局办公室加成": f"+{全局办公室加成:.1f}%" if 全局办公室加成 > 0 else "+0%",
            "总联络速度": f"+{总联络速度:.1f}%",
